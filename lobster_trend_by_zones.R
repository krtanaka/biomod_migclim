...
for (j in 1:length(season)){
  ...
  df <- read_csv(paste0("Biomod_1_80_", season[j],".csv")) #lobster fall biomod output
  ...
  p = ggplot(data, aes(x = x, y = y, color = Management_Area)) +
    ...
    labs(x = "Model Year", y = "Relative Habitat Suitability") + ggtitle(paste0("American_lobster_", season[j]))
  p
  
  png(paste0("/Users/Kisei/Desktop/lobster_change_", season[j], ".png"), res = 500, height = 10, width = 2.5, units = "in")
  print(p)
  dev.off()
  
  write_csv(r_df, paste0("/Users/kisei/Desktop/lobster_", season[j], "_ensemble_area_trend.csv"))
