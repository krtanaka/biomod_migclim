...
for (j in 1:length(season)){
  ...
  data$Model = gsub("scallop_AllData_RUN", "", data$Model)
  ...
  coef$Model = gsub("scallop_AllData_RUN", "", coef$Model)
  ...
  p = ggplot(data, aes(x = x, y = y, color = Model)) +
    ...
    geom_line( data = subset( data, Model %in% 'Ensemble' ),
               ...
               show.legend = FALSE,
               size = I( 1 )) +
    # geom_point( data = subset( data, Model %in% 'Ensemble' ), size = I( 0.8 ) ) +
    # geom_point( data = subset( data, ! ( Model %in% 'Ensemble' ) ), size = I( 0.4 ) ) +
    # geom_smooth( data = subset( data, Model %in% 'Ensemble'), aes( group = 1 ), size = I( 0.4 ) ) +
    # geom_smooth( data = subset( data, Model %in% 'Ensemble'), size = I( 0.4 ), method = "lm", se = T ) +
    # geom_smooth( data = subset( data, ! ( Model %in% 'Ensemble' ) ), size = I( 0.4 ), method = "lm", se = T ) +
  ...
  save(data, file = paste0("/Users/Kisei/Desktop/scallop_change_", season[[j]], ".RData"))
  
  png(paste0("/Users/Kisei/Desktop/scallop_change_", season[j], ".png"), 
  ...
  write_csv(sdm, paste0("/Users/kisei/Desktop/scallop_", season[j], "_sdm_area_agreement.csv"))
